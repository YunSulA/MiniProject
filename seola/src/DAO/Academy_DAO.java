...
public class Academy_DAO {
	...
	public int addDao(Connection conn, Academy academy) throws SQLException {
		...
		try {	// sno. name. birth. tel. address. major. tribute. datascore.javascore.javascriptscore.jspscore.attendance.
			...
			pstmt=conn.prepareStatement("INSERT INTO ACADEMY VALUES (4, ' ', ' ', ' ', ' ', ' ', 1,1,1,1,1, 1.5, '설문')");
...
//	}
	...
	public int changeNameDao(Connection conn, Academy academy) throws SQLException {
		...
		try {
			...
			pstmt.setInt(2, Academy.button_check);	// 몇번 버튼(몇번 캐릭)이 이름변경을 요청한건지 확인. 설아가 sno만들었다고 했는데 아직 정확히 파악못해서 일단 내가 만든 button_check 로 확인 
			return pstmt.executeUpdate();
		} finally {
			DBResourceReturn.close(pstmt);
		}
...
//	}
	...
	public void selectDao(Connection conn, Academy academy) throws SQLException {
		...
		try {
			...
			while (rs.next()) {
	// sno. name. birth. tel. address. major. tribute. datascore.javascore.javascriptscore.jspscore.attendance.
				
				
			/* 여기에 있는 int sno , String name, 이런 것들은 지금 임의로 내가 만든 변수명들이고,
			      코드에 적용할때는 Ui 패널에 실제로 정보를 출력해야하는 곳에 rs.get~ 를 적용해 바로 출력이 되게 하면 될 것 같다. */
